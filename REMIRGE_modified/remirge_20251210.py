#!/usr/bin/env python2
"""
Bo Zhao modified in 2025/12/10,debuged for resume function and 'N in reconstructed sequences' issue
...
import sys
sys.path.append('/newdatabase/galaxy_8084/EMIRGE/')
import os
import re
import csv
...
BOWTIE2_N = 1
BOWTIE2_L = 20
BOWTIE2_ASCII_OFFSET = 33   # currently, bowtie2 writes quals with an ascii offset of 33

sys.path.append("/newdatabase/HEMIRGE/galaxy/tools/HEMIRGE")
class Record:
    """
...
class EM(object):
    ...
    def __init__(self, reads1_filepath, reads2_filepath,
        ...
        n_cpus is how many processors to use for multithreaded steps (currently only the bowtie2 mapping)
    ...
    def read_bam(self, bam_filename, reference_fasta_filename):
        ...
        ascii_offset = BOWTIE2_ASCII_OFFSET
        ...
        seq_i, read_i = _emirge.process_bamfile_predata(seq_i, read_i,
                                                        ...
                                                        self.coverage, BOWTIE2_ASCII_OFFSET,
                                                        self.bamfile_data)
    ...
    def resume(self, resume_from):
        ...
        if not os.path.exists(os.path.join(resume_iterdir, "bowtie2.iter.%02d.log.gz"%(self.resume_i))):
            raise OSError, "\n\nERROR: directory %s appears to be an incomplete iteration (no bowtie2 log file).  You must resume from a *completed* iteration (perhaps try iteration %02d)."%(resume_iterdir, self.resume_i-1)
        if self._VERBOSE:
    ...
    def save_state(self, filename = None):
        ...
        save state
        """
        tup_to_save = (self.bamfile_data, self.base2i, self.cluster_thresh, self.coverage, self.current_bam_filename, self.current_reference_fasta_filename, self.cwd, self.i2base, self.insert_sd, self.iteration_i, self.iterdir, self.iterdir_prefix, self.k, self.likelihoods, self.mapping_nice, self.min_depth, self.min_length_coverage, self.min_prior, self.n_cpus, self.posteriors, self.priors, self.probN, self.quals, self.read_i2read_name, self.read_name2read_i, self.reads1_filepath, self.reads2_filepath, self.sequence_i2sequence_name, self.sequence_name2fasta_name, self.sequence_name2sequence_i, self.snp_minor_prob_thresh, self.snp_percentage_thresh, self.unmapped_bases, self.v)
        if filename is None:
    ...
    def load_state(self, filename = None):
        ...
        for name in ("bamfile_data", "base2i", "cluster_thresh", "coverage", "current_bam_filename", "current_reference_fasta_filename", "cwd", "i2base", "insert_sd", "iteration_i", "iterdir", "iterdir_prefix", "k", "likelihoods", "mapping_nice", "min_depth", "min_length_coverage", "min_prior", "n_cpus", "posteriors", "priors", "probN", "quals", "read_i2read_name", "read_name2read_i", "reads1_filepath", "reads2_filepath", "sequence_i2sequence_name", "sequence_name2fasta_name", "sequence_name2sequence_i", "snp_minor_prob_thresh", "snp_percentage_thresh", "unmapped_bases", "v"):
            if not hasattr(self, name):
                setattr(self, name, None)
        try:
            (self.bamfile_data, self.base2i, self.cluster_thresh, self.coverage, self.current_bam_filename, self.current_reference_fasta_filename, self.cwd, self.i2base, self.insert_sd, self.iteration_i, self.iterdir, self.iterdir_prefix, self.k, self.likelihoods, self.mapping_nice, self.min_depth, self.min_length_coverage, self.min_prior, self.n_cpus, self.posteriors, self.priors, self.probN, self.quals, self.read_i2read_name, self.read_name2read_i, self.reads1_filepath, self.reads2_filepath, self.sequence_i2sequence_name, self.sequence_name2fasta_name, self.sequence_name2sequence_i, self.snp_minor_prob_thresh, self.snp_percentage_thresh, self.unmapped_bases, self.v) = \
                                cPickle.load(infile)
        except ValueError:  # old version didn't have bamfile_data
            (self.base2i, self.cluster_thresh, self.coverage, self.current_bam_filename, self.current_reference_fasta_filename, self.cwd, self.i2base, self.insert_sd, self.iteration_i, self.iterdir_prefix, self.k, self.likelihoods, self.mapping_nice, self.min_depth, self.min_length_coverage, self.min_prior, self.n_cpus, self.posteriors, self.priors, self.probN, self.quals, self.read_i2read_name, self.read_name2read_i, self.reads1_filepath, self.reads2_filepath, self.sequence_i2sequence_name, self.sequence_name2fasta_name, self.sequence_name2sequence_i, self.snp_minor_prob_thresh, self.snp_percentage_thresh, self.unmapped_bases, self.v) = \
                                cPickle.load(infile)
        self.fastafile = pysam.Fastafile(self.current_reference_fasta_filename)
    ...
    def do_mapping_bowtie2(self, full_fasta_path, nice = None):
        ...
        """
        bowtie2_logfile = os.path.join(self.iterdir, "bowtie2.iter.%02d.log"%(self.iteration_i))
        bowtie2_index   = os.path.join(self.iterdir, "bowtie2.index.iter.%02d"%(self.iteration_i))
        # 1. build index
        cmd = "bowtie2-build -o 3 %s %s > %s 2>&1"%(full_fasta_path , bowtie2_index, bowtie2_logfile) # -o 3 for speed? magnitude of speedup untested!
        if self._VERBOSE:
        ...
        shared_bowtie2_params = " --no-unal --phred%d -p %s -D %s -R %s -N %s -L %s"%(self.reads_ascii_offset, self.n_cpus, BOWTIE2_D, BOWTIE2_R, BOWTIE2_N, BOWTIE2_L)

        output_prefix = os.path.join(self.iterdir, "bowtie2.iter.%02d"%(self.iteration_i))
        if self.reads2_filepath is not None:
            bowtie2_command = " %s %s | %s bowtie2 %s -1 - -2 %s -x %s | python3 /mnt/data/home/tycloud/tool/anaconda3/envs/emirge/bin/sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.PE.bam "%(\
                cat_cmd,
                self.reads1_filepath,
                nicestring,
                shared_bowtie2_params,
                self.reads2_filepath,
                bowtie2_index,
                output_prefix)
        else: # single reads
            bowtie2_command = " %s %s | %s bowtie2 %s -U - -x %s | python3 /mnt/data/home/tycloud/tool/anaconda3/envs/emirge/bin/sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.PE.bam "%(\
                cat_cmd,
                self.reads1_filepath,
                nicestring,
                shared_bowtie2_params,
        ...
        if self._VERBOSE:
            sys.stderr.write("\tbowtie2 command:\n")
            sys.stderr.write("\t%s\n"%bowtie2_command)

        check_call(bowtie2_command, shell=True, stdout = sys.stdout, stderr = sys.stderr)

        if self._VERBOSE:
            sys.stderr.write("\tFinished Bowtie2 for iteration %02d at %s:\n"%(self.iteration_i, ctime()))

        # 3. clean up
        # check_call("samtools index %s.sort.PE.bam"%(output_prefix), shell=True, stdout = sys.stdout, stderr = sys.stderr)
        check_call("gzip -f %s"%(bowtie2_logfile), shell=True)

        assert self.iterdir != '/'
        for filename in os.listdir(self.iterdir):
            assert(len(os.path.basename(bowtie2_index)) >= 20)  # weak check that I'm not doing anything dumb.
            if os.path.basename(bowtie2_index) in filename:
                os.remove(os.path.join(self.iterdir, filename))
        return
...
def test_generic(cwd, bam_ref, fasta_ref,
    ...
    """
    INITIAL BOWTIE2 COMMAND (right now run separately.  This reports 1 alignment per read.):
    """
...
def do_iterations(em, max_iter, save_every):
    ...
    while em.iteration_i < max_iter:
        ...
        em.do_iteration(os.path.join(subdir, "bowtie2.iter.%02d.PE.bam"%(em.iteration_i)),
...
def do_initial_mapping(working_dir, options):
    ...
    does the initial 1-reference-per-read bowtie2 mapping to initialize the algorithm
    ...
    bampath_prefix = os.path.join(initial_mapping_dir, "initial_bowtie2_mapping.PE")
    ...
    option_strings.extend([options.fastq_reads_1, nicestring, reads_ascii_offset, options.processors, BOWTIE2_D, BOWTIE2_R, BOWTIE2_N, BOWTIE2_L])

    # PAIRED END MAPPING shared_bowtie2_params = "--no-unal --phred%d -p %s -D %s -R %s -N %s -L %s "%(self.reads_ascii_offset, self.n_cpus, BOWTIE2_D, BOWTIE2_R, BOWTIE2_N, BOWTIE2_L)
    if options.fastq_reads_2 is not None:
        option_strings.extend([options.fastq_reads_2, options.bowtie2_db, bampath_prefix])
        cmd = " %s %s | %s bowtie2 --no-unal --phred%d -p %s -D %s -R %s -N %s -L %s -1 - -2 %s -x %s | python3 /mnt/data/home/tycloud/tool/anaconda3/envs/emirge/bin/sam_filter.py /dev/stdin /dev/stdout| samtools view -b -S -u -F 0x0004 - > %s.bam "%tuple(option_strings)
    # SINGLE END MAPPING
    else:
        option_strings.extend([options.bowtie2_db, bampath_prefix])
        cmd = " %s %s | %s bowtie2 --no-unal --phred%d -p %s -D %s -R %s -N %s -L %s -U - -x %s | python3 /mnt/data/home/tycloud/tool/anaconda3/envs/emirge/bin/sam_filter.py /dev/stdin /dev/stdout | samtools view -b -S -u -F 0x0004 - > %s.bam "%tuple(option_strings)
...
def main(argv = sys.argv[1:]):
    ...
    group_opt.add_option("-j", "--join_threshold",
                      ...
                      help="If two candidate sequences share >= this fractional identity over their bases with mapped reads, then merge the two sequences into one for the next iteration.  (default: %default; valid range: [0.5, 1.0] ) ")
    ...
    if options.join_threshold < 0.5 or options.join_threshold > 1:
...
PloS one 8: e56018. doi:10.1371/journal.pone.0056018.\n\n""")
    ...
    if options.bowtie2_db is None:
        if options.fasta_db:
            parser.error("Bowtie2 index is missing (--bowtie2_db). You need to build it before running EMIRGE\nTry:\n\nbowtie2-build %s bowtie2_prefix" % options.fasta_db)
        else:
            parser.error("Bowtie2 index is missing (--bowtie2_db). You need to build it before running EMIRGE\nTry:\n\nbowtie2-build candidate_db.fasta bowtie2_prefix")
    ...
    if options.fastq_reads_1 is None: 
    ...
    for o in ["fastq_reads_1", "fastq_reads_2", "fasta_db", "bowtie2_db", "mapping"]:
